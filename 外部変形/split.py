...
def line_judge(p1, p2, qx, qy):
    ...
    print("線上判定：" + str((qy * (p1x - p2x)) +
          (p1y * (p2x - qx)) + (p2y * (qx - p1x))))
# 点qが線分上にあればTrueを返す
...
##### 区画割の実行#####
def parcel_allocation(deep_make_road_edge, make_road_edge, parcel_frame, road_width, road_edge, maguchi, goal_area):
    ...
    for k in range(len(road_edge)):
        ...
        home_cnt = int(road_distance / maguchi)
        real_maguchi = road_distance / home_cnt
        home_depth = goal_area / real_maguchi
        # 間口毎のベクトルの算出
        print("calc:" + str(road_edge[k][1][0] - road_edge[k][0][0]))
        deviation = [(road_edge[k][1][0] - road_edge[k][0][0]) / home_cnt,
                     (road_edge[k][1][1] - road_edge[k][0][1]) / home_cnt]
        ...
        double_multiple = (home_depth * home_depth) / \
            (deviation[0] * deviation[0] + deviation[1] * deviation[1])
        multiple = math.sqrt(double_multiple)
        # 奥行座標の格納
        right_coor_x, right_coor_y = road_edge[k][0][0] - deviation[1] * \
            multiple, road_edge[k][0][1] + deviation[0] * multiple
        # 奥行座標と間口間ベクトルで構成された直線と枠線の交点（二点）を算出
        for i in range(1, len(parcel_frame)):
            intersection = line_cross_point(parcel_frame[i - 1], parcel_frame[i], [
                                            right_coor_x, right_coor_y], [right_coor_x + deviation[0], right_coor_y + deviation[1]])
        ...
        print("home_cnt:" + str(home_cnt) + " real_maguchi:" +
              str(real_maguchi) + " home_depth:" + str(home_depth))
        # 結果リストに格納
        result.append([[inter_coor[0][0], inter_coor[0][1]],
                      [inter_coor[1][0], inter_coor[1][1]]])
        # 各家の座標を格納(間口間ベクトル参照)
        for i in range(1, home_cnt):
            ...
            if line_judge(cp_inter_coor[0], cp_inter_coor[1], cp_inter_coor[0][0] + (i * deviation[0]), cp_inter_coor[0][1] + (i * deviation[1])):
                ...
                instant_coor_b.append(
                    instant_coor_a[0] - deviation[1] * multiple)
                instant_coor_b.append(
                    instant_coor_a[1] + deviation[0] * multiple)
            ...
            if exist_flag:
                if k % 2 == 0:
                    if i == 1:
                        evaluation.append([eva_coor[0], eva_coor[1], [
                                          inter_coor[0][0], inter_coor[0][1]], parcel_frame[0]])
                    elif i == home_cnt - 1:
                        evaluation.append(
                            [eva_coor[i * 2 - 2], eva_coor[i * 2 - 1], eva_coor[i * 2 - 3], eva_coor[i * 2 - 4]])
                        evaluation.append([parcel_frame[1], [inter_coor[1][0], inter_coor[1][1]], eva_coor[(
                            i - 1) * 2 - 1], eva_coor[(i - 1) * 2]])
                        break
                    else:
                        evaluation.append(
                            [eva_coor[i * 2 - 2], eva_coor[i * 2 - 1], eva_coor[i * 2 - 3], eva_coor[i * 2 - 4]])
                    ...
                    if i == 1:
                        evaluation.append(
                            [[inter_coor[0][0], inter_coor[0][1]], parcel_frame[2], eva_coor[0], eva_coor[1]])
                    elif i == home_cnt - 1:
                        evaluation.append(
                            [eva_coor[i * 2 - 3], eva_coor[i * 2 - 4], eva_coor[i * 2 - 2], eva_coor[i * 2 - 1]])
                        evaluation.append([parcel_frame[3], [inter_coor[1][0], inter_coor[1][1]], eva_coor[(
                            i - 1) * 2 - 1], eva_coor[(i - 1) * 2]])
                        break
                    else:
                        evaluation.append(
                ...
                if k % 2 == 0:
                    evaluation.append([parcel_frame[1], [inter_coor[1][0], inter_coor[1][1]], eva_coor[(
                        i - 1) * 2 - 2], eva_coor[(i - 1) * 2 - 1]])
                    break
                else:
                    evaluation.append([eva_coor[(i - 1) * 2 - 1], eva_coor[(i - 1) * 2 - 2], [
                                      inter_coor[1][0], inter_coor[1][1]], parcel_frame[3]])
                    break
            exist_flag = False
        eva_coor = []
        # 街区を小さくしていく処理
    ...
    for i in range(0, (len(parcel_frame) - 1), 1):
        ...
        if line_judge(make_road_edge[0], make_road_edge[1], parcel_frame[i][0], parcel_frame[i][1]):
            if line_judge(make_road_edge[0], make_road_edge[1], parcel_frame[i + 1][0], parcel_frame[i + 1][1]):
                ...
                make_road_edge[1] = [parcel_frame[i + 1]
                                     [0], parcel_frame[i + 1][1]]
                # 該当街区のため，置き換え実行
                break
    # 道の場所を決定する（道作成辺の半分地点）
    ...
    # 道作成辺の中点を基準に道の入り口を算出
    start_road = [(make_road_edge[0][0] + make_road_edge[1][0]) / 2,
                  (make_road_edge[0][1] + make_road_edge[1][1]) / 2]
    ...
    road_tilt, road_intercept = line_formula(
        make_road_edge[0], make_road_edge[1])
    ...
    make_road_edge.append(
        [start_road[0] + (x * constant_k), start_road[1] + (y * constant_k)])
    make_road_edge.append(
        [start_road[0] - (x * constant_k), start_road[1] - (y * constant_k)])
    print("make_road_edge:" + str(make_road_edge))
    # 道の最大の長さ，最小の長さの決定
    for i in range(1, len(parcel_frame)):
        ...
        intersection = line_cross_point(
            parcel_frame[i - 1], parcel_frame[i], start_road, make_road_edge[2])
        print("intersection:" + str(intersection))
        # 交点を追加
        if intersection is not None:
    ...
    while not inside_flag:
        # 最大の長さの半分以上でランダムに生成
        road_length = random.uniform(
            road_inter_distance / 2, road_inter_distance)
        randam_constant_k = road_length / math.sqrt(x * x + y * y)
        print("randam_constant_k:" + str(randam_constant_k))
        # 道の端となる二点を生成
        inside_coor_1 = [make_road_edge[3][0] - y * randam_constant_k,
                         make_road_edge[3][1] + x * randam_constant_k]
        if inside_polygon(inside_coor_1, list(parcel_frame[:-1])):
            # 内部にある
            inside_flag_1 = True
        # 二点目の生成
        inside_coor_2 = [make_road_edge[4][0] - y * randam_constant_k,
                         make_road_edge[4][1] + x * randam_constant_k]
        ...
        if inside_flag_1 and inside_flag_2:
            inside_flag = True
    # 行き止まりの先に中線を描画
    road_mid_line.append(line_cross_point(
        inside_coor_1, inside_coor_2, road_inter_coor[0], road_inter_coor[1]))
    # ここの座標選択を定式化しないといけない！！！！！
...
##### 上区画の分割#####
    ...
    area_calc = []
    # 道に沿って間口ベクトルの算出
    deviation = [(parcel_frame[1][0] - make_road_edge[4][0]),
                 (parcel_frame[1][1] - make_road_edge[4][1])]
    parcel_deviation = [(parcel_frame[0][0] - parcel_frame[1][0]),
                        (parcel_frame[0][1] - parcel_frame[1][1])]
    road_deviation = [(make_road_edge[4][0] - inside_coor_2[0]),
                      (make_road_edge[4][1] - inside_coor_2[1])]
    deviation_k = maguchi / \
        math.sqrt(deviation[0] * deviation[0] + deviation[1] * deviation[1])
    instant_coor_b = [make_road_edge[4][0] + deviation[1] *
                      deviation_k, make_road_edge[4][1] - deviation[0] * deviation_k]
    instant_coor_a = line_cross_point(parcel_frame[0], parcel_frame[1], instant_coor_b, [
                                      instant_coor_b[0] - road_deviation[1] * 5, instant_coor_b[1] + road_deviation[0] * 5])
    ...
    if instant_coor_a is None:
        ...
        instant_coor_b = [instant_coor_a[0] -
                          deviation[0], instant_coor_a[1] - deviation[1]]
    ...
    if not goal_area_flag:
        ...
        while True:
            ...
            expand_distance = (goal_area - home_area +
                               2000000) / expand_area_distance
            expand_k = expand_distance / \
                math.sqrt(
                    road_deviation[0] * road_deviation[0] + road_deviation[1] * road_deviation[1])
            # 角度を変えずに平行移動
            instant_coor_b = [instant_coor_b[0] - road_deviation[0] *
                              expand_k, instant_coor_b[1] - road_deviation[1] * expand_k]
            instant_coor_a = line_cross_point(parcel_frame[0], parcel_frame[1], instant_coor_b, [
                                              instant_coor_b[0] - road_deviation[1] * 5, instant_coor_b[1] + road_deviation[0] * 5])
    ...
    # 道の奥側の処理
    instant_coor_b = [road_mid_line[1][0] - deviation[1] *
                      deviation_k, road_mid_line[1][1] + deviation[0] * deviation_k]
    # 直角の座標を入手
    x = make_road_edge[1][0] - make_road_edge[0][0]
    y = make_road_edge[1][1] - make_road_edge[0][1]
    instant_coor_a = line_cross_point(parcel_frame[0], parcel_frame[1], instant_coor_b, [
                                      instant_coor_b[0] - x * 2, instant_coor_b[1] - y * 2])
    print("各座標:" + str(instant_coor_a) + " " + str(instant_coor_b))
    # 指定の座標（上）がparcel内にあるかどうか
    if instant_coor_a is None:
        instant_coor_a = parcel_frame[0]
        non_road = [road_mid_line[0][0] - inside_coor_1[0],
                    road_mid_line[0][1] - inside_coor_1[1]]
        instant_coor_b = [instant_coor_a[0] -
                          deviation[0], instant_coor_a[1] - deviation[1]]
        print("各座標:" + str(instant_coor_a) + " " + str(instant_coor_b))
    ...
    if not line_judge(road_mid_line[0], road_mid_line[1], instant_coor_b[0], instant_coor_b[1]):
        ...
        instant_coor_b = [instant_coor_b[0] -
                          (x * constant_k), instant_coor_b[1] - (y * constant_k)]
    ...
    if not goal_area_flag:
        ...
        while True:
            ...
            expand_distance = (goal_area - home_area +
                               2000000) / expand_area_distance
            expand_k = expand_distance / \
                math.sqrt(deviation[0] * deviation[0] +
                          deviation[1] * deviation[1])
            print("各要素:" + str(instant_coor_a) + ", " + str(instant_coor_b))
            instant_coor_b = [instant_coor_b[0] - deviation[1] *
                              expand_k, instant_coor_b[1] + deviation[0] * expand_k]
            instant_coor_a = line_cross_point(parcel_frame[0], late_parcel[0], instant_coor_b, [
                                              instant_coor_b[0] + deviation[0] * 5, instant_coor_b[1] + deviation[1] * 5])
            ...
            if instant_coor_a is None:
                if oku_kukaku_count == 1:
                    instant_coor_b = [road_mid_line[1][0] - deviation[1] * deviation_k *
                                      1.5, road_mid_line[1][1] + deviation[0] * deviation_k * 1.5]
                    instant_coor_a = line_cross_point(parcel_frame[0], late_parcel[0], instant_coor_b, [
                                                      instant_coor_b[0] + deviation[0] * 5, instant_coor_b[1] + deviation[1] * 5])
                    road_frame_flag = False
                    print("各要素:" + str(instant_coor_a) +
                          ", " + str(instant_coor_b))
                else:
                    break_flag = False
                    break
            ...
            if not road_frame_flag:
                if not line_judge(road_mid_line[0], road_mid_line[1], instant_coor_b[0], instant_coor_b[1]):
                    if not instant_b_inroad:
                        # 指定座標（下）を進入経路上に移動
                        instant_coor_b = [
                            instant_coor_b[0] - (x * constant_k), instant_coor_b[1] - (y * constant_k)]
                    else:
                ...
                if road_frame_flag and inside_parcel_flag:
                    ...
                    area_calc.append(inside_coor_2)
                    area_calc.append(
                        [road_mid_line[0][0], road_mid_line[0][1]])
                    area_calc.append(
                        [road_mid_line[1][0], road_mid_line[1][1]])
                    area_calc.append(parcel_frame[0])
                    area_calc.append(instant_coor_a)
                elif not road_frame_flag and inside_parcel_flag:
                    area_calc.append(instant_coor_a)
    ...
    if len(late_parcel) == 3:
        ...
        # 複数の家が立てられる場合
            ...
            late_deviation = [(late_parcel[2][0] - late_parcel[1][0]) /
                              home_cnt, (late_parcel[2][1] - late_parcel[1][1]) / home_cnt]
            # 家の戸数分ループ
            for i in range(1, home_cnt):
                instant_coor_b = [late_parcel[1][0] + late_deviation[0]
                                  * i, late_parcel[1][1] + late_deviation[1] * i]
                instant_coor_a = line_cross_point(parcel_frame[0], parcel_frame[1], instant_coor_b, [
                                                  instant_coor_b[0] + deviation[0] * 5, instant_coor_b[1] + deviation[1] * 5])
                result.append([instant_coor_a, instant_coor_b])
                # 算出した区画毎に評価用配列に格納
                if i == 1:
                    evaluation.append(
                        [late_parcel[0], late_parcel[1], instant_coor_b, instant_coor_a])
                elif i == home_cnt - 1:
                    evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                      late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
                    evaluation.append(
                        [instant_coor_a, instant_coor_b, late_parcel[2]])
                else:
                    evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                      late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
        ...
        print("late_parcel:" + str(late_parcel))
        print("################## " + str(road_distance) + " " + str(maguchi))
        # 家が作れない場合は終了
        if home_cnt == 0:
            print("もう家が作れません")
        # 一つだけ作成可能の場合
            ...
            for i in range(1, home_cnt):
                ...
                if i == 1:
                    ...
                    evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                      late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
                    evaluation.append(
                        [instant_coor_a, instant_coor_b, late_parcel[2], late_parcel[3]])
                else:
                    evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                      late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
        ...
        if home_cnt == 0:
            ...
            late_deviation = [(late_parcel[4][0] - late_parcel[0][0]) /
                              home_cnt, (late_parcel[4][1] - late_parcel[0][1]) / home_cnt]
            for i in range(1, home_cnt):
                instant_coor_a = [late_parcel[0][0] + late_deviation[0]
                                  * i, late_parcel[0][1] + late_deviation[1] * i]
                instant_coor_b = line_cross_point(late_parcel[1], late_parcel[2], instant_coor_a, [
                                                  instant_coor_a[0] - deviation[0] * 5, instant_coor_a[1] - deviation[1] * 5])
                print("指定座標範囲判定・instant_coor_b:" + str(instant_coor_b))
                if instant_coor_b is None:
                    instant_coor_b = line_cross_point(late_parcel[3], late_parcel[4], instant_coor_a, [
                                                      instant_coor_a[0] - deviation[0] * 5, instant_coor_a[1] - deviation[1] * 5])
                    print("指定座標範囲判定・instant_coor_b:" + str(instant_coor_b))
                result.append([instant_coor_a, instant_coor_b])
                # 座標の追加・評価関数配列に格納
                if line_judge(late_parcel[1], late_parcel[2], instant_coor_b[0], instant_coor_b[1]):
                    if i == 1:
                        ...
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
                        evaluation.append(
                            [instant_coor_a, instant_coor_b, late_parcel[2], late_parcel[3], late_parcel[4]])
                    else:
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
                else:
                    if i == 1:
                        evaluation.append([late_parcel[0], late_parcel[1], inside_coor_2, [
                                          road_mid_line[0][0], road_mid_line[0][1]], instant_coor_b, instant_coor_a])
                    elif i == home_cnt - 1:
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [late_parcel[1][0] - late_deviation[0] * (home_cnt - i + 1), late_parcel[1][1] - late_deviation[1] * (
                            home_cnt - i + 1)], inside_coor_2, [road_mid_line[0][0], road_mid_line[0][1]], [late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
                        evaluation.append(
                            [instant_coor_a, instant_coor_b, late_parcel[4]])
                    else:
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [late_parcel[1][0] - late_deviation[0] * (home_cnt - i + 1), late_parcel[1][1] - late_deviation[1] * (
                            home_cnt - i + 1)], inside_coor_2, [road_mid_line[0][0], road_mid_line[0][1]], [late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
        ...
        # 複数の宅地を作成できる場合
            ...
            late_deviation = [(late_parcel[5][0] - late_parcel[0][0]) /
                              home_cnt, (late_parcel[5][1] - late_parcel[0][1]) / home_cnt]
            for i in range(1, home_cnt):
                instant_coor_a = [late_parcel[0][0] + late_deviation[0]
                                  * i, late_parcel[0][1] + late_deviation[1] * i]
                instant_coor_b = line_cross_point(late_parcel[1], late_parcel[2], instant_coor_a, [
                ...
                if line_judge(make_road_edge[4], inside_coor_2, instant_coor_b[0], instant_coor_b[1]):
                    if i == 1:
                        ...
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
                        evaluation.append(
                            [instant_coor_a, instant_coor_b, late_parcel[2], late_parcel[3], late_parcel[4], late_parcel[5]])
                    else:
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
                else:
                    if i == 1:
                        ...
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [late_parcel[1][0] - late_deviation[0] * (home_cnt - i + 1), late_parcel[1][1] - late_deviation[1] * (
                            home_cnt - i + 1)], inside_coor_2, [road_mid_line[0][0], road_mid_line[0][1]], [late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
                        evaluation.append(
                            [instant_coor_a, instant_coor_b, late_parcel[4], late_parcel[5]])
                    else:
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [late_parcel[1][0] - late_deviation[0] * (home_cnt - i + 1), late_parcel[1][1] - late_deviation[1] * (
                            home_cnt - i + 1)], inside_coor_2, [road_mid_line[0][0], road_mid_line[0][1]], [late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_b, instant_coor_a])
        ### 汎用的な記述ではない###
...
##### 下区画の分割#####
    ...
    area_calc = []
    # 道に沿って間口ベクトルの算出
    deviation = [(parcel_frame[2][0] - make_road_edge[3][0]),
                 (parcel_frame[2][1] - make_road_edge[3][1])]
    parcel_deviation = [(parcel_frame[3][0] - parcel_frame[2][0]),
                        (parcel_frame[3][1] - parcel_frame[2][1])]
    road_deviation = [(make_road_edge[3][0] - inside_coor_1[0]),
                      (make_road_edge[3][1] - inside_coor_2[1])]
    deviation_k = maguchi / \
        math.sqrt(deviation[0] * deviation[0] + deviation[1] * deviation[1])
    instant_coor_b = [make_road_edge[3][0] - deviation[1] *
                      deviation_k, make_road_edge[3][1] + deviation[0] * deviation_k]
    instant_coor_a = line_cross_point(parcel_frame[3], parcel_frame[2], instant_coor_b, [
                                      instant_coor_b[0] + deviation[0] * 3, instant_coor_b[1] + deviation[1] * 3])
    print("                                    #")
    ...
    if not goal_area_flag:
        ...
        while True:
            ...
            instant_coor_b = [instant_coor_b[0] - road_deviation[0] *
                              expand_k, instant_coor_b[1] - road_deviation[1] * expand_k]
            instant_coor_a = line_cross_point(parcel_frame[2], parcel_frame[3], instant_coor_b, [
                                              instant_coor_b[0] - road_deviation[1] * 5, instant_coor_b[1] + road_deviation[0] * 5])
            if line_judge(make_road_edge[3], inside_coor_1, instant_coor_b[0], instant_coor_b[1]):
                ...
                non_in_road = [road_mid_line[0][0] - inside_coor_1[0],
                               road_mid_line[0][1] - inside_coor_1[1]]
                instant_coor_b = [
                    instant_coor_b[0] + non_in_road[0], instant_coor_b[1] + non_in_road[1]]
                instant_area.append(instant_coor_b)
    ...
    instant_coor_a = line_cross_point(late_parcel[1], parcel_frame[3], instant_coor_b, [
                                      instant_coor_b[0] - x * 2, instant_coor_b[1] - y * 2])
    non_in_road = [inside_coor_1[0] - road_mid_line[0]
                   [0], inside_coor_1[1] - road_mid_line[0][1]]
    print("各座標:" + str(instant_coor_a) + ", " + str(instant_coor_b))
    ...
    if not line_judge(road_mid_line[0], road_mid_line[1], instant_coor_b[0], instant_coor_b[1]):
        print("進入経路上")
        road_frame_flag = True
        instant_coor_b = [instant_coor_b[0] + non_in_road[0],
                          instant_coor_b[1] + non_in_road[1]]
    # 4パターンの結果についてそれぞれ面積を求める
    if road_frame_flag and inside_parcel_flag:
        area_calc.append(instant_coor_b)
        area_calc.append(instant_coor_a)
        ...
        while True:
            ...
            if instant_coor_a is None:
                if oku_kukaku_count == 1 and not road_frame_flag:
                    instant_coor_b = [
                        road_mid_line[1][0] + non_in_road[0], road_mid_line[1][1] + non_in_road[0]]
                    instant_coor_a = line_cross_point(limit_coor, parcel_frame[3], instant_coor_b, [
                                                      instant_coor_b[0] + deviation[0] * 5, instant_coor_b[1] + deviation[1] * 5])
                    road_frame_flag = True
                else:
                    break
    ...
    if len(late_parcel) == 3:
        ...
        # 複数の宅地を作成できる場合
            ...
            late_deviation = [(late_parcel[2][0] - late_parcel[0][0]) /
                              home_cnt, (late_parcel[2][1] - late_parcel[0][1]) / home_cnt]
            # 家の戸数分ループ
            for i in range(1, home_cnt):
                instant_coor_b = [late_parcel[0][0] + late_deviation[0]
                                  * i, late_parcel[0][1] + late_deviation[1] * i]
                instant_coor_a = line_cross_point(parcel_frame[1], parcel_frame[2], instant_coor_b, [
                                                  instant_coor_b[0] + deviation[0] * 5, instant_coor_b[1] + deviation[1] * 5])
                result.append([instant_coor_a, instant_coor_b])
                # 算出した区画毎に評価用配列に格納
                if i == 1:
        ...
        # 複数の家が建てられる場合
            ...
            late_deviation = [(late_parcel[3][0] - late_parcel[0][0]) /
                              home_cnt, (late_parcel[3][1] - late_parcel[0][1]) / home_cnt]
            # 家の戸数分ループ
            for i in range(1, home_cnt):
                instant_coor_b = [late_parcel[0][0] + late_deviation[0]
                                  * i, late_parcel[0][1] + late_deviation[1] * i]
                ...
                if i == 1:
                    evaluation.append(
                        [late_parcel[0], late_parcel[1], instant_coor_a, instant_coor_b])
                elif i == home_cnt - 1:
                    evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                      late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_a, instant_coor_b])
                    evaluation.append(
                        [instant_coor_b, instant_coor_a, late_parcel[2], late_parcel[3]])
                else:
                    evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                      late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_a, instant_coor_b])
    elif len(late_parcel) == 5:
        ...
        # 複数の宅地が作成可能な場合
            ...
            late_deviation = [(late_parcel[2][0] - late_parcel[1][0]) /
                              home_cnt, (late_parcel[2][1] - late_parcel[1][1]) / home_cnt]
            for i in range(1, home_cnt):
                instant_coor_a = [late_parcel[1][0] + late_deviation[0]
                                  * i, late_parcel[1][1] + late_deviation[1] * i]
                instant_coor_b = line_cross_point(late_parcel[0], late_parcel[4], instant_coor_a, [
                                                  instant_coor_a[0] - deviation[0] * 5, instant_coor_a[1] - deviation[1] * 5])
                print("指定座標範囲判定・instant_coor_b:" + str(instant_coor_b))
                if instant_coor_b is None:
                    instant_coor_b = line_cross_point(late_parcel[2], late_parcel[3], instant_coor_a, [
                                                      instant_coor_a[0] - deviation[0] * 5, instant_coor_a[1] - deviation[1] * 5])
                    print("指定座標範囲判定・instant_coor_b:" + str(instant_coor_b))
                # 座標の追加・評価関数配列に格納
                result.append([instant_coor_a, instant_coor_b])
                if line_judge(late_parcel[0], late_parcel[4], instant_coor_b[0], instant_coor_b[1]):
                    if i == 1:
                        ...
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_a, instant_coor_b])
                        evaluation.append(
                            [instant_coor_b, instant_coor_a, late_parcel[2], late_parcel[3], late_parcel[4]])
                    else:
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_a, instant_coor_b])
                else:
                    if i == 1:
                        ...
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_a, instant_coor_b])
                        evaluation.append(
                            [instant_coor_b, instant_coor_a, late_parcel[2]])
                    else:
                        evaluation.append([[late_parcel[0][0] + late_deviation[0] * (i - 1), late_parcel[0][1] + late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] + late_deviation[0] * (i - 1), late_parcel[1][1] + late_deviation[1] * (i - 1)], instant_coor_a, instant_coor_b])
    elif len(late_parcel) == 6:
        ...
        # 複数の宅地を作成できる場合
            ...
            for i in range(1, home_cnt):
                instant_coor_a = [late_parcel[1][0] + late_deviation[0]
                                  * i, late_parcel[1][1] + late_deviation[1] * i]
                instant_coor_b = line_cross_point(late_parcel[0], late_parcel[5], instant_coor_a, [
                                                  instant_coor_a[0] - deviation[0] * 5, instant_coor_a[1] - deviation[1] * 5])
                print("指定座標範囲判定・instant_coor_b:" + str(instant_coor_b))
                if instant_coor_b is None:
                    instant_coor_b = line_cross_point(late_parcel[3], late_parcel[4], instant_coor_a, [
                ...
                if line_judge(late_parcel[0], late_parcel[5], instant_coor_b[0], instant_coor_b[1]):
                    ...
                    if i == 1:
                        ...
                        evaluation.append([[late_parcel[0][0] - late_deviation[0] * (i - 1), late_parcel[0][1] - late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] - late_deviation[0] * (i - 1), late_parcel[1][1] - late_deviation[1] * (i - 1)], instant_coor_a, instant_coor_b])
                        evaluation.append(
                            [instant_coor_b, instant_coor_a, late_parcel[2], late_parcel[3], late_parcel[4], late_parcel[5]])
                    else:
                        evaluation.append([[late_parcel[0][0] - late_deviation[0] * (i - 1), late_parcel[0][1] - late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] - late_deviation[0] * (i - 1), late_parcel[1][1] - late_deviation[1] * (i - 1)], instant_coor_a, instant_coor_b])
                else:
                    if i == 1:
                        evaluation.append([late_parcel[0], late_parcel[1], instant_coor_a, instant_coor_b, [
                                          road_mid_line[0][0], road_mid_line[0][1]], inside_coor_1])
                    elif i == home_cnt - 1:
                        evaluation.append([[late_parcel[0][0] - late_deviation[0] * (i - 1), late_parcel[0][1] - late_deviation[1] * (i - 1)], [
                                          late_parcel[1][0] - late_deviation[0] * (i - 1), late_parcel[1][1] - late_deviation[1] * (i - 1)], instant_coor_a, instant_coor_b])
                        evaluation.append(
    ...
    # 評価の実行
    detail_list, total_score, total_score_margin = evaluate_calc.eval(
        fin_road_edge, cp_evaluation, road_width, goal_area)
...
if line_count == 6:
    ...
    for line in info:
        ...
        # 道作成辺の読み込み
            ...
            make_road_edge = [[float(xy[0]), float(xy[1])], [
    ...
    for i in range(30):
        ...
        # 区画割の実行
        result, detail_list, total_score, evaluation, total_score_margin = parcel_allocation(
            deep_make_road_edge, make_road_edge, cp_frame, road_width, road_edge, maguchi, goal_area)
        # リスト化
        result = list(result)
        # 結果をそれぞれのindexに追加
    ...
    print(index_index)
    # 同じ得点の得点と個数を抽出
    material = [k for k, v in collections.Counter(
        index_index).items() if v > 1]
    count = [v for k, v in collections.Counter(index_index).items() if v > 1]
    # 同じ得点のものは小さい加点で差をつける
    for i in range(len(material)):
        for j in range(count[i]):
            print(score_index.index(material[i]))
            score_index[score_index.index(
                material[i])] = score_index[score_index.index(material[i])] + 0.0001 * j
    # 点数を調整して再度任意の個数を抽出
    index_index = []
    cp_result_index = []
    cp_detail_index = []
    ...
    for i in range(1, 21, 1):
        ...
        cp_evaluation_index.append(
            evaluation_index[score_index.index(target_num)])
    ...
    for i in range(len(material)):
        for j in range(count[i]):
            ...
            score_index_margin[score_index_margin.index(
                material[i])] = score_index_margin[score_index_margin.index(material[i])] + 0.0001 * j
    # 点数を調整して再度任意の個数を抽出
    index_index = []
    cp_result_index = []
    cp_detail_index = []
    ...
    for i in range(1, 21, 1):
        ...
        target_num = sorted(score_index_margin)[-i]
        index_index.append(target_num)
        cp_result_index.append(
            result_index[score_index_margin.index(target_num)])
        cp_detail_index.append(
            detail_index[score_index_margin.index(target_num)])
        cp_score_index_margin.append(
            score_index_margin[score_index_margin.index(target_num)])
        cp_evaluation_index.append(
            evaluation_index[score_index_margin.index(target_num)])
    ...
    for j in range(5):
        for k in range(4):
            ...
            for i in range(len(write_result)):
                ...
                write_result[i][0] = list(write_result[i][0])
                write_result[i][1] = list(write_result[i][1])
                write_result[i][0][0] = write_result[i][0][0] - \
                    (j + 2) * 100000
                write_result[i][1][0] = write_result[i][1][0] - \
                    (j + 2) * 100000
                write_result[i][0][1] = write_result[i][0][1] + k * 80000
                write_result[i][1][1] = write_result[i][1][1] + k * 80000
                msp.add_line(start=write_result[i][0], end=write_result[i][1])
    ...
    print(" ")
    print("maguchi:" + str(maguchi))
    print("least_maguchi:" + str(least_maguchi))
    print("goal_area:" + str(goal_area))
    print("frame:" + str(frame))
    print("road_edge:" + str(road_edge))
    print(" ")
    ...
    for i in range(100):
        ...
        # 区画割の実行
        # result, point_list, total_score, evaluation = un_load.unload_parcel_allocation(cp_frame, road_edge, maguchi, least_maguchi, goal_area)
        result, point_list, total_score, evaluation = un_load_binary.unload_parcel_allocation(
            cp_frame, road_edge, maguchi, least_maguchi, goal_area)
        # リスト化
        result = list(result)
        # 結果をそれぞれのindexに追加
        result_index.append(result)
