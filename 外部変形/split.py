...
def line_judge(p1, p2, qx, qy):
    ...
    print(
        "線上判定：" + str((qy * (p1x - p2x)) + (p1y * (p2x - qx)) + (p2y * (qx - p1x)))
    )
    # 点qが線分上にあればTrueを返す
...
def parcel_allocation(
    ...
    for k in range(len(road_edge)):
        ...
        deviation = [
            (road_edge[k][1][0] - road_edge[k][0][0]) / home_cnt,
        ...
        # 奥行座標の格納
        right_coor_x, right_coor_y = (
            road_edge[k][0][0] - deviation[1] * multiple,
            road_edge[k][0][1] + deviation[0] * multiple,
        )
        ...
        for i in range(1, len(parcel_frame)):
            intersection = line_cross_point(
                ...
                parcel_frame[i],
                [right_coor_x, right_coor_y],
                [right_coor_x + deviation[0], right_coor_y + deviation[1]],
            )
        ...
        # 結果リストに格納
        result.append(
            [[inter_coor[0][0], inter_coor[0][1]], [inter_coor[1][0], inter_coor[1][1]]]
        )
        # 各家の座標を格納(間口間ベクトル参照)
        for i in range(1, home_cnt):
            ...
            if line_judge(
                cp_inter_coor[0],
                cp_inter_coor[1],
                cp_inter_coor[0][0] + (i * deviation[0]),
                cp_inter_coor[0][1] + (i * deviation[1]),
            ):
                exist_flag = True
                ...
                cp_inter_coor[0],
                cp_inter_coor[1],
                cp_inter_coor[1][0] + (i * deviation[0]),
                cp_inter_coor[1][1] + (i * deviation[1]),
            ):
                exist_flag = True
                instant_coor_a.append(road_edge[k][0][0] + (i * deviation[0]))
                instant_coor_a.append(road_edge[k][0][1] + (i * deviation[1]))
    ...
    for i in range(0, (len(parcel_frame) - 1), 1):
        # 道作成辺を狭めた街区の座標に置き換え
        if line_judge(
            make_road_edge[0], make_road_edge[1], parcel_frame[i][0], parcel_frame[i][1]
        ):
            if line_judge(
                make_road_edge[0],
                make_road_edge[1],
    ...
    # 道作成辺の中点を基準に道の入り口を算出
    start_road = [
        (make_road_edge[0][0] + make_road_edge[1][0]) / 2,
    ...
    while not inside_flag:
        ...
        # 道の端となる二点を生成
        inside_coor_1 = [
            make_road_edge[3][0] - y * randam_constant_k,
        ...
        # 二点目の生成
        inside_coor_2 = [
            make_road_edge[4][0] - y * randam_constant_k,
        ...
        if inside_flag_1 and inside_flag_2:
            inside_flag = True
    # 行き止まりの先に中線を描画
    road_mid_line.append(
        line_cross_point(
            inside_coor_1, inside_coor_2, road_inter_coor[0], road_inter_coor[1]
        )
    )
    # ここの座標選択を定式化しないといけない！！！！！
    if not road_inter_coor[0] == line_cross_point(
        make_road_edge[0], make_road_edge[1], start_road, make_road_edge[2]
    ):
    ...
    # 道に沿って間口ベクトルの算出
    deviation = [
        (parcel_frame[1][0] - make_road_edge[4][0]),
        (parcel_frame[1][1] - make_road_edge[4][1]),
    ]
    parcel_deviation = [
        (parcel_frame[0][0] - parcel_frame[1][0]),
        (parcel_frame[0][1] - parcel_frame[1][1]),
    ]
    road_deviation = [
        (make_road_edge[4][0] - inside_coor_2[0]),
        (make_road_edge[4][1] - inside_coor_2[1]),
    ]
    deviation_k = maguchi / math.sqrt(
        deviation[0] * deviation[0] + deviation[1] * deviation[1]
    )
    instant_coor_b = [
        make_road_edge[4][0] + deviation[1] * deviation_k,
        make_road_edge[4][1] - deviation[0] * deviation_k,
    ]
    instant_coor_a = line_cross_point(
        parcel_frame[0],
        parcel_frame[1],
        instant_coor_b,
        [
            instant_coor_b[0] - road_deviation[1] * 5,
            instant_coor_b[1] + road_deviation[0] * 5,
        ],
    )
    ...
    if instant_coor_a is None:
        ...
        instant_coor_b = [
            instant_coor_a[0] - deviation[0],
            instant_coor_a[1] - deviation[1],
        ]
    ...
    if not goal_area_flag:
        ...
        while True:
            ...
            expand_k = expand_distance / math.sqrt(
                road_deviation[0] * road_deviation[0]
                + road_deviation[1] * road_deviation[1]
            )
            # 角度を変えずに平行移動
            instant_coor_b = [
                instant_coor_b[0] - road_deviation[0] * expand_k,
                instant_coor_b[1] - road_deviation[1] * expand_k,
            ]
            instant_coor_a = line_cross_point(
                parcel_frame[0],
                parcel_frame[1],
    ...
    # 道の奥側の処理
    instant_coor_b = [
        road_mid_line[1][0] - deviation[1] * deviation_k,
        road_mid_line[1][1] + deviation[0] * deviation_k,
    ]
    # 直角の座標を入手
    x = make_road_edge[1][0] - make_road_edge[0][0]
    y = make_road_edge[1][1] - make_road_edge[0][1]
    ...
    if instant_coor_a is None:
        ...
        non_road = [
            road_mid_line[0][0] - inside_coor_1[0],
    ...
    if not line_judge(
        road_mid_line[0], road_mid_line[1], instant_coor_b[0], instant_coor_b[1]
    ):
        print("進入経路上")
        road_frame_flag = True
        instant_coor_b = [
            instant_coor_b[0] - (x * constant_k),
            instant_coor_b[1] - (y * constant_k),
        ]
    # road_mid_lineがparcel_frameのどの辺上にあるかで場合分け
    decrease_parcel_flag = False
    if not line_judge(
        parcel_frame[0], parcel_frame[3], road_mid_line[1][0], road_mid_line[1][1]
    ):
    ...
    if not goal_area_flag:
        ...
        while True:
            ...
            instant_coor_b = [
                instant_coor_b[0] - deviation[1] * expand_k,
                instant_coor_b[1] + deviation[0] * expand_k,
            ]
            instant_coor_a = line_cross_point(
                parcel_frame[0],
                late_parcel[0],
                instant_coor_b,
                [
                    instant_coor_b[0] + deviation[0] * 5,
                    instant_coor_b[1] + deviation[1] * 5,
                ],
            )
            ...
            if instant_coor_a is None:
                if oku_kukaku_count == 1:
                    instant_coor_b = [
                        road_mid_line[1][0] - deviation[1] * deviation_k * 1.5,
                        road_mid_line[1][1] + deviation[0] * deviation_k * 1.5,
                    ]
                    instant_coor_a = line_cross_point(
                        parcel_frame[0],
                        late_parcel[0],
    ...
    if len(late_parcel) == 3:
        ...
        # 複数の家が立てられる場合
            ...
            late_deviation = [
                (late_parcel[2][0] - late_parcel[1][0]) / home_cnt,
                (late_parcel[2][1] - late_parcel[1][1]) / home_cnt,
            ]
            # 家の戸数分ループ
            for i in range(1, home_cnt):
                instant_coor_b = [
                    late_parcel[1][0] + late_deviation[0] * i,
                    late_parcel[1][1] + late_deviation[1] * i,
                ]
                instant_coor_a = line_cross_point(
                    parcel_frame[0],
                    parcel_frame[1],
                ...
                if i == 1:
                    ...
                    evaluation.append(
                        [
                            [
                                late_parcel[0][0] + late_deviation[0] * (i - 1),
                                late_parcel[0][1] + late_deviation[1] * (i - 1),
                            ],
                            [
                                late_parcel[1][0] + late_deviation[0] * (i - 1),
                                late_parcel[1][1] + late_deviation[1] * (i - 1),
                            ],
                            instant_coor_b,
                            instant_coor_a,
                        ]
        ...
        if home_cnt == 0:
            ...
            late_deviation = [
                (late_parcel[4][0] - late_parcel[0][0]) / home_cnt,
                (late_parcel[4][1] - late_parcel[0][1]) / home_cnt,
            ]
            for i in range(1, home_cnt):
                instant_coor_a = [
                    late_parcel[0][0] + late_deviation[0] * i,
                    late_parcel[0][1] + late_deviation[1] * i,
                ]
                instant_coor_b = line_cross_point(
                    late_parcel[1],
                    late_parcel[2],
                    instant_coor_a,
                    [
                        instant_coor_a[0] - deviation[0] * 5,
                        instant_coor_a[1] - deviation[1] * 5,
                    ],
                )
                ...
                # 座標の追加・評価関数配列に格納
                if line_judge(
                    late_parcel[1], late_parcel[2], instant_coor_b[0], instant_coor_b[1]
                ):
                    if i == 1:
                        evaluation.append(
                            [
                    ...
                    if i == 1:
                        ...
                        evaluation.append(
                            [
                                ...
                                [
                                    late_parcel[1][0]
                                    - late_deviation[0] * (home_cnt - i + 1),
                                    late_parcel[1][1]
                                    - late_deviation[1] * (home_cnt - i + 1),
                                ],
                                inside_coor_2,
                                [road_mid_line[0][0], road_mid_line[0][1]],
                                [
        ...
        # 複数の宅地を作成できる場合
            ...
            late_deviation = [
                (late_parcel[5][0] - late_parcel[0][0]) / home_cnt,
                (late_parcel[5][1] - late_parcel[0][1]) / home_cnt,
            ]
            for i in range(1, home_cnt):
                instant_coor_a = [
                    late_parcel[0][0] + late_deviation[0] * i,
    ...
    # 道に沿って間口ベクトルの算出
    deviation = [
        (parcel_frame[2][0] - make_road_edge[3][0]),
        (parcel_frame[2][1] - make_road_edge[3][1]),
    ]
    parcel_deviation = [
        (parcel_frame[3][0] - parcel_frame[2][0]),
        (parcel_frame[3][1] - parcel_frame[2][1]),
    ]
    road_deviation = [
        (make_road_edge[3][0] - inside_coor_1[0]),
    ...
    instant_coor_b = [
        make_road_edge[3][0] - deviation[1] * deviation_k,
        make_road_edge[3][1] + deviation[0] * deviation_k,
    ]
    instant_coor_a = line_cross_point(
        parcel_frame[3],
        parcel_frame[2],
    ...
    if not goal_area_flag:
        ...
        while True:
            ...
            if line_judge(
                make_road_edge[3], inside_coor_1, instant_coor_b[0], instant_coor_b[1]
            ):
                instant_area.append(instant_coor_b)
                ...
                instant_coor_b = [
                    instant_coor_b[0] + non_in_road[0],
    ...
    non_in_road = [
        inside_coor_1[0] - road_mid_line[0][0],
        inside_coor_1[1] - road_mid_line[0][1],
    ]
    print("各座標:" + str(instant_coor_a) + ", " + str(instant_coor_b))
        ...
        while True:
            ...
            if instant_coor_a is None:
                if oku_kukaku_count == 1 and not road_frame_flag:
                    instant_coor_b = [
                        road_mid_line[1][0] + non_in_road[0],
                        road_mid_line[1][1] + non_in_road[0],
                    ]
                    instant_coor_a = line_cross_point(
                        limit_coor,
                        parcel_frame[3],
    ...
    if len(late_parcel) == 3:
        ...
        # 複数の宅地を作成できる場合
            ...
            late_deviation = [
                (late_parcel[2][0] - late_parcel[0][0]) / home_cnt,
                (late_parcel[2][1] - late_parcel[0][1]) / home_cnt,
            ]
            # 家の戸数分ループ
            for i in range(1, home_cnt):
                instant_coor_b = [
        ...
        # 複数の家が建てられる場合
            ...
            late_deviation = [
                (late_parcel[3][0] - late_parcel[0][0]) / home_cnt,
                (late_parcel[3][1] - late_parcel[0][1]) / home_cnt,
            ]
            # 家の戸数分ループ
            for i in range(1, home_cnt):
                instant_coor_b = [
        ...
        # 複数の宅地が作成可能な場合
            ...
            for i in range(1, home_cnt):
                ...
                result.append([instant_coor_a, instant_coor_b])
                if line_judge(
                    late_parcel[0], late_parcel[4], instant_coor_b[0], instant_coor_b[1]
                ):
                    if i == 1:
                        evaluation.append(
                            [
        ...
        # 複数の宅地を作成できる場合
            ...
            for i in range(1, home_cnt):
                ...
                # 座標の追加・評価関数配列に格納
                if line_judge(
                    late_parcel[0], late_parcel[5], instant_coor_b[0], instant_coor_b[1]
                ):
                    # result.append([instant_coor_a, instant_coor_b])
                    if i == 1:
                        evaluation.append(
...
if line_count == 6:
    ...
    for i in range(30):
        ...
        # 区画割の実行
        result, detail_list, total_score, evaluation, total_score_margin = (
            parcel_allocation(
                deep_make_road_edge,
                make_road_edge,
                cp_frame,
    ...
    for i in range(len(material)):
        for j in range(count[i]):
            ...
            score_index[score_index.index(material[i])] = (
                score_index[score_index.index(material[i])] + 0.0001 * j
            )
    # 点数を調整して再度任意の個数を抽出
    index_index = []
    cp_result_index = []
    ...
    for i in range(len(material)):
        for j in range(count[i]):
            ...
            score_index_margin[score_index_margin.index(material[i])] = (
                score_index_margin[score_index_margin.index(material[i])] + 0.0001 * j
            )
    # 点数を調整して再度任意の個数を抽出
    index_index = []
    cp_result_index = []
    ...
    for i in range(100):
        ...
        result, point_list, total_score, evaluation = (
            un_load_binary.unload_parcel_allocation(
